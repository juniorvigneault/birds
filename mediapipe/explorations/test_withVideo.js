...
let sketch = new p5(function (p5) {
  ...
  p5.draw = function () {
    ...
    if (objectDetector && isDetecting) {
      ...
      // }
      // frames++
      birdsDetected = results.detections.filter(detection => detection.categories[0].categoryName === 'bird');
  ...
  function createBirdImage(box) {
    ...
    let birdImage = birdFootage.videoFeed.get(box.originX - 15, box.originY - 15, box.width + 30, box.height + 30);
  ...
  function displayBirdCam(bird) {
    ...
    bird.mask(circleMask);
    p5.imageMode(p5.CENTER);
    p5.image(bird, p5.width / 2, p5.height / 2 + circleMaskSize / 2 - 54, circleMaskSize, circleMaskSize);
  }
